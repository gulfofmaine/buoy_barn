...
from deployments.tasks import refresh_dataset as refresh_dataset_base
...
def not_recently_refreshed_datasets(time_before: timedelta) -> Iterable[int]:
    ...
    old_datasets = ErddapDataset.objects.filter(refresh_attempted__lt=older_than) | ErddapDataset.objects.filter(refresh_attempted__isnull=True)
...
def refreshed_datasets_status(dataset_ids: Iterable[int]):
    ...
    refreshed_datasets = ErddapDataset.objects.select_related("server").filter(id__in=dataset_ids).order_by("server__name", "name")
