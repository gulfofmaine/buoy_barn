...
class ErddapLoaderTestCase(TestCase):
    ...
    def test_load_dataset_from_erddap_and_create_timeseries(self):
        ...
        for ts in self.platform.timeseries_set.all():
            ...
            self.assertEqual(ts.erddap_dataset, dataset)
            self.assertEqual(ts.erddap_server, self.erddap_server)
            self.assertIn(ts.variable, ('significant_wave_height', 'dominant_wave_period'))
